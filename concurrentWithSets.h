...
void SetsConcurrent<T>::resize() {
    ...
    while(resizing){
        ...
        table1 = std::vector<std::shared_ptr<std::unordered_set<T>>>(maxSize);
        table2 = std::vector<std::shared_ptr<std::unordered_set<T>>>(maxSize);
...
template <typename T>
bool SetsConcurrent<T>::add_safe(T value) {
    int h0 = t1Hash(value) % maxSize, h1 = t2Hash(value) % maxSize;
    int i = -1, h = -1;
    bool mustResize = false;
...
template <typename T>
bool SetsConcurrent<T>::relocate_safe(int i, int hi) {
    int hj = 0;
    int j = 1 - i;
    int LIMIT = 10;
